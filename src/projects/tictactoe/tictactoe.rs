...
fn board(ref_pass_array: &mut [[char; 3]; 3]) {
...
fn check_available_or_not(
    ref_pass_array: &mut [[char; 3]; 3],
    row: i32,
    column: i32,
    player: char,
) {
...
fn init() {
    ...
    let mut board_3d_array: [[char; 3]; 3] = [[' ', ' ', ' '], [' ', ' ', ' '], [' ', ' ', ' ']];
    let mut count = 1;
    let mut player = Player::X;

    while count < 10 {
        let user_choice = take_user_input();
        inserting_char_in_board(&mut board_3d_array, user_choice, player);
        //clear_screen();
...
fn inserting_char_in_board(mut arr_3d: &mut [[char; 3]; 3], inp: i32, player: Player) {
    match inp {
        1 => {
            arr_3d[0][0] = 'X';
            check_available_or_not(&mut arr_3d, 0, 0, 'X');
        }
        2 => arr_3d[0][1] = '0',
        3 => arr_3d[0][2] = 'X',
        4 => arr_3d[1][0] = '0',

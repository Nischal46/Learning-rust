...
fn main() {
    ...
    //doing asynchronous concept

    println!("This is first");

    //this is synchronaus behhaviour
    // let _async_for_2sec = std::thread::sleep(std::time::Duration::from_secs(3));

    let handle_for_two_seconds = std::thread::spawn(move || {
        std::thread::sleep(std::time::Duration::from_secs(2));
        println!("This is asynchronous behhaviour");
    });

    println!("This is blocked for 2 sec");
